...
class MySql:
		...
		try:
			...
			cmd = """
				CREATE TABLE `calibre2mysql`.`{table}` ( 
					...
					`cal_collection` VARCHAR(20) NOT NULL,
					`cal_book_id` INTEGER NOT NULL,
					`cal_auth_id` INTEGER NOT NULL,
					`cal_book_name` VARCHAR(500) NOT NULL , 
					`cal_author_name` VARCHAR(500) NOT NULL ,
					`cal_path` VARCHAR(500) NOT NULL,
					`cal_name` VARCHAR(500) NOT NULL,
					`cal_formats` VARCHAR(100) NOT NULL,
					PRIMARY KEY (`cal_id`)
				) ENGINE = MyISAM;
	...
	def save_rows(self, rows):
		...
		cmd = """
			INSERT INTO `{table}` 
			(`cal_collection`,`cal_book_id`,`cal_auth_id`,`cal_book_name`,`cal_author_name`,`cal_path`,`cal_name`,`cal_formats`) 
